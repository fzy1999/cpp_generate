...
public:
  explicit Action(Context *ctx) : _ast_callback(ctx), _ctx(ctx) {
    static const auto class_matcher =
        cxxRecordDecl(isDefinition(), unless(isExpansionInSystemHeader()))
            .bind("c");
    _finder.addMatcher(class_matcher, &_ast_callback);
    static const auto enum_matcher =
        enumDecl(unless(isExpansionInSystemHeader())).bind("e");
    _finder.addMatcher(enum_matcher, &_ast_callback);
    static const auto method_matcher =
        cxxMethodDecl(isDefinition(), unless(isExpansionInSystemHeader()))
            .bind("m");
    _finder.addMatcher(method_matcher, &_ast_callback);
  }
  ...
  CreateASTConsumer(CompilerInstance &compiler,
    ...
    compiler.getPreprocessor().addPPCallbacks(
        std::make_unique<MacroCallback>(compiler.getSourceManager(), //
